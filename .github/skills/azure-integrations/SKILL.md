...
- User asks general questions about Azure without specific integration tasks
- Request is for AWS, Google Cloud, or other cloud providers
- Task doesn't involve deploying or configuring Azure resources
- User is looking for basic cloud concepts without implementation
- Simple web development without Azure deployment needs
...
### Deployment Workflow
```yaml
# .github/workflows/azure-app-service.yml (see full example in examples/)
...
```bash
# Recommended: Use Key Vault Reference pattern
...
```bash
# ❌ Avoid: Exposes MongoDB and auth secrets in app settings
az webapp config appsettings set --name <app-name> --resource-group <rg> --settings \
